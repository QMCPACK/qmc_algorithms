...
class GTO:
  def __init__(self, basis=None, center=[0.0, 0.0, 0.0]):
    ...
    self.gto_lambda = dict()
    nmax = 3
    for i in range(nmax):
      for j in range(nmax):
        for k in range(nmax):
          self.gto_lambda[(i,j,k)] = lambdify((x,y,z,alpha),self.gto_sym.subs(self.make_min_subs_list(i,j,k)).evalf())
    #gto000_str = lambdastr((x,y,z,alpha),self.gto_sym.subs(self.make_min_subs_list(0,0,0)).evalf())
    #print gto000_str
  ...
  def make_min_subs_list(self, i, j, k):
      subs_list = {self.i:i, self.j:j, self.k:k}
      r2 = self.x**2 + self.y**2 + self.z**2
      r_val = sqrt(r2)
      subs_list[self.r] = r_val
  ...
  def eval_single_v(self, i, j, k, x, y, z, alpha):
      ...
      v = self.gto_lambda[(i,j,k)](xc,yc,zc,alpha)
      return v
...
class MO:
  def __init__(self, gto, coeff):
    self.gto = gto
    self.coeff = coeff

  def eval_v(self, x, y, z):
    v = self.gto.eval_v(x, y, z)
    return np.dot(self.coeff, np.array(v,dtype='float64'))

  def eval_vgl(self, x, y, z):
    v,g,l = self.gto.eval_vgl(x, y, z)
    mo_v =  np.dot(self.coeff, np.array(v,dtype='float64'))
    mo_g =  np.dot(self.coeff, np.array(g,dtype='float64'))
    mo_l =  np.dot(self.coeff, np.array(l,dtype='float64'))
